...
experiment_name = "use_elu_after_deleteMaxPool_and_reducelearn_5_layer_conv"
NUM_CLASSES = 10
BATCH_SIZE = 40
...
with tf.name_scope('conv-1') as scope:
    ...
    local1 = tf.nn.elu(pre_activation,name="local_relu_"+scope)
    # pool1
    pool1=[]
...
with tf.name_scope('conv-2') as scope:
    ...
    conv2 = tf.nn.conv2d(prev_layer, kernel2, [1, 1, 1, 1], padding='SAME',name="conv_"+scope)
    #biases = _variable_on_cpu('biases', [64], tf.constant_initializer(0.0))
    biases2 = tf.Variable(tf.zeros([conv1_out_size]), name='biases_'+scope) 
    pre_activation2 = tf.nn.bias_add(conv2, biases2)
    # conv1 = tf.nn.relu(pre_activation)
    local2 = tf.nn.elu(pre_activation2, name='local_relu_'+scope)
    # pool1
...
with tf.name_scope('conv-3') as scope:
    ...
    # conv1 = tf.nn.relu(pre_activation)
    local3 = tf.nn.elu(pre_activation3, name='local_relu_'+scope)
    # pool1
...
with tf.name_scope('conv-4') as scope:
    ...
    conv4 = tf.nn.conv2d(local3, kernel4, [1, 1, 1, 1], padding='SAME',name="conv_"+scope)
    #biases = _variable_on_cpu('biases', [64], tf.constant_initializer(0.0))
    biases4 = tf.Variable(tf.zeros([conv1_out_size]), name='biases_'+scope) 
    pre_activation4 = tf.nn.bias_add(conv4, biases4)
    # conv1 = tf.nn.relu(pre_activation)
    local4 = tf.nn.elu(pre_activation4, name='local_relu_'+scope)
    # pool1
...
with tf.name_scope('conv-5') as scope:
    ...
    conv5 = tf.nn.conv2d(local4, kernel5, [1, 1, 1, 1], padding='SAME',name="conv_"+scope)
    #biases = _variable_on_cpu('biases', [64], tf.constant_initializer(0.0))
    biases5 = tf.Variable(tf.zeros([conv1_out_size]), 'biases', name='biases_'+scope) 
    pre_activation5 = tf.nn.bias_add(conv5, biases5)
    # conv1 = tf.nn.relu(pre_activation)
    local5 = tf.nn.elu(pre_activation5, name='local_relu_'+scope)
    # pool1
